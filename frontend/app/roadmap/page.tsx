  ...
  useEffect(() => {
    ...
    async function loadRoadmapData() {
      ...
      try {
        ...
        if (isMounted) {
          setError(err instanceof Error ? err.message : "Không thể tải dữ liệu roadmap.");
        }
      } finally {
        if (isMounted) {
          setIsFetching(false);
          ...
          <form onSubmit={handleGenerateRoadmap} className="mt-6 space-y-4">
            ...
            <label className="block text-sm font-medium text-slate-200">
              ...
              <input
                ...
                placeholder="Ví dụ: 1 tháng, 2 tháng, 6 tuần"
            ...
            {selectedAnalysis ? (
              <div className="rounded-md border border-white/10 bg-slate-950/60 p-4 text-sm leading-6 text-slate-300">
                ...
                <p className="mt-2">Match score: {selectedAnalysis.match_score}%</p>
                <p className="mt-1">Skill gap: {selectedAnalysis.skill_gap_summary}</p>
              </div>
            ) : (
              <div className="rounded-md border border-amber-300/20 bg-amber-300/10 p-4 text-sm leading-6 text-amber-100">
                Không chọn analysis thì backend sẽ dùng career profile để tạo roadmap basic. Hãy cập nhật profile nếu bạn chưa nhập target role, skill và timeline.
              </div>
            )}
      ...
      <section className="mx-auto w-full max-w-6xl px-4 pb-10 sm:px-6">
        <div className="rounded-lg border border-white/10 bg-white/5 p-5 sm:p-6">
          ...
          <div className="mt-4 space-y-4">
            {roadmaps.length === 0 ? (
              <p className="text-sm text-slate-400">Chưa có roadmap nào.</p>
            ) : (
...
function EmptyRoadmap() {
  return (
    <div className="rounded-lg border border-white/10 bg-white/5 p-6">
      ...
      <p className="mt-2 text-sm leading-6 text-slate-300">
        Chọn analysis gần đây hoặc dùng career profile, nhập timeline nếu cần, rồi tạo roadmap học tập ngắn hạn có action cụ thể theo từng tuần.
      </p>
    </div>
  );
}
    ...
    <article className="min-w-0 rounded-lg border border-white/10 bg-slate-950/60 p-5">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        <div className="min-w-0">
          <p className="text-xs font-semibold uppercase tracking-[0.18em] text-cyan-300">{title ?? "Learning Roadmap"}</p>
      ...
      <p className="mt-4 text-sm leading-6 text-slate-300">{roadmap.summary}</p>
    ...
    <section className="rounded-md border border-white/10 bg-white/5 p-4">
      <div className="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between">
        <div>
          ...
          <h4 className="mt-1 text-base font-semibold text-slate-100">{item.focus}</h4>
        </div>
      </div>

      <div className="mt-4">
        <p className="text-sm font-semibold text-slate-200">Skills cần học</p>
      ...
      <div className="mt-4">
        ...
        <ul className="mt-2 space-y-2 text-sm leading-6 text-slate-300">
          {item.actions.map((action) => (
            <li key={action} className="rounded-md border border-white/10 bg-slate-950/70 p-3">{action}</li>
          ))}
        </ul>
      </div>
